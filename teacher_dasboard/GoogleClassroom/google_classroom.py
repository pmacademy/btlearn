...
async def list_google_courses(teacher_id:str,db: Session = Depends(get_db)):
...
async def connect_google_courses(teacher_id:str, course_ids: Optional[List[str]]=Query(None), db: Session = Depends(get_db)):
